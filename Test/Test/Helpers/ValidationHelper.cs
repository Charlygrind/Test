...
namespace Test.Helpers
{
    public static class ValidationHelper
    {
        ...
        private static void ShowValidationFields(List<ValidationResult> errors, object model, Page page, string validationLabelSuffix = "Error")
        {
            ...
            {
                var memberName = error.ErrorMessage.Equals("Las contraseñas no coinciden") ? $"{model.GetType().Name}_{"ConfirmPassword"}" : $"{model.GetType().Name}_{error.MemberNames.FirstOrDefault()}";
